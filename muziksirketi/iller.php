<?php
include("baglan.php");
$sorgu = $db->prepare('SELECT * FROM encokkonservereniller');
$sorgu->execute();
$encokkonserverenillerlist = $sorgu->fetchAll(PDO::FETCH_OBJ);

$query1 = $db->query('SELECT * FROM encokkonserverenbolgeler');
         ...
         <script type="text/javascript">
            ...
            data.addRows([
                  ...
                  foreach($query1 as $row1) {
                     echo "['".$row1["bolge_ad"]."',".$row1["konserSayisi"]."],";
                  }
                  ?>
            ]);
            var options = {'title':'',
                           'width':500,
                           'height':400};
            var chart = new google.visualization.PieChart(document.getElementById('eckvb_chart'));
        ...
        <div class="content">
            <div class="animated fadeIn">
                <div class="row">
                    <div class="col-lg-6">
                        <div class="card">
                            <div class="card-header"><strong>EN ÇOK KONSER VERİLEN BÖLGELER</strong></div>
                            <div class="card-body card-block">
                                <div id="eckvb_chart"></div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <strong class="card-title">İLLER</strong>
                            </div>
                            <div class="card-body">
                                <table class="table table-striped table-bordered">
                                    ...
                                    <?php foreach ($encokkonserverenillerlist as $eckvl){?>
                                    <tbody>
                                        <tr>
                                            <td><?= $eckvl->il_ad ?></td>
                                            <td><?= $eckvl->konserSayisi ?></td>
                                        </tr>
                                    </tbody>
                                    <?php } ?>
                                </table>
